...
```{r}
PE<-abs((mod1_serie_pib$x-mod1_serie_pib$fitted)/mod1_serie_pib$x)
...
```{r}
mod2_serie_pib<-HoltWinters(serie_pib_ent,gamma = FALSE)
mod2_serie_pib
```
...
```{r}
PE<-abs((mod2_serie_pib$x-mod2_serie_pib$fitted)/mod2_serie_pib$x)
MAPE<-sum(PE)/length(PE)
MAPE*100
```
...
```{r}
plot(serie_exportacion,main="Indice global de la actividad económica IGAE\n Bolivia (Ene 2008 a Julio 2021)",
     xlab="Gestión",ylab="IGAE")
```
